...
function DoctorDetail(){
    ...
    React.useEffect(() => {
      ...
      axios.get(`http://localhost:5000/api/doctor/slots/${docData.uid}/${date}`)
        .then(function(response){
        
       console.log("PROCEDURE REPONSE");
    ...
    React.useEffect(() => {
        ...
        axios.get(`http://localhost:5000/api/doctor/private/${params.uid}`,{
            headers: {
      ...
      if(((parseInt(date.substring(8)) + 1).toString()).length == 1)
         date1 = date.substring(0,8).concat("0").concat((parseInt(date.substring(8))).toString());
        else 
        date1 = date.substring(0,8).concat((parseInt(date.substring(8))).toString());
        console.log(date1);
        console.log(reason1);
        setDate(date1);
        axios.post("http://localhost:5000/api/doctor/book/appointments",{
            d_uid:docData.uid,
        ...
        <Grid container spacing={3}>
                ...
                <Grid item xs={4}>
        <Typography variant='body1'>{docData.uid}</Typography>
                </Grid>
                <Grid item xs={2}>
                </Grid>
         </Grid>
