...
function Hospital_New(){
    ...
    React.useEffect(() => {
        if(role == "Univ"){
          axios.get("https://dataproj-backend-api.herokuapp.com/api/univ/hospital/" + localStorage.getItem('uid'))     
...
const handleSubmit = (event) =>{
  ...
  axios.get(`https://dataproj-backend-api.herokuapp.com/api/univ/search/hosp/${localStorage.getItem('uid')}/` + String(dataRef.current.value))
  .then(function(response){
    console.log("hi");
    ...
    return(
        ...
        <br />
           ...
           <form>
              <label>
                Name:
                <input type="text" name="name" align = "center" style={{fontSize: "2.0vw", color: "#11223E", marginLeft: "15vw" }} ref={dataRef}/>
              </label>
