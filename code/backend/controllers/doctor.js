...
exports.fetchSlots = (req,res,next) =>{
    
    const doc_id = req.params.id;
    const date_app = req.params.doa;
    allDoctors.fetchSlotsById(doc_id, date_app)
    .then(result => {
        if(result[0].length != 0)
        {
...
exports.getVaccFormById = (req,res,next) =>{
    const id = req.params.id;
    allDoctors.fetchVaccFormById(id)
    .then(result => {
     if(result[0].length != 0)
     {
...
exports.UpdateVaccById = (req,res,next) =>{
    const id = req.params.id;
    allDoctors.updateVaccById(id)
    .then(
     res.status(200).json({
         success:1,
